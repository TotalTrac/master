...
GO
CREATE PROCEDURE [dbo].[ClientsGetAssigneesAutoComplete]
	@ID				int 
...
IF @CLIENT_TYPE = 16 OR @CLIENT_TYPE IS NULL
  BEGIN
	...
	WHERE
		...
		OR CLIENT_SHORTNAME LIKE '%' + @Substring + '%') AND (c.CLIENT_TYPE | 8 <> 0))
    END
ELSE
  BEGIN
  	SELECT DISTINCT
	...
	FROM 
		...
		INNER JOIN [dbo].[ClientRelationships] AS r
			ON (c.CLIENT_ID = r.PartnerClientId
				AND r.ParentClientId = @Client_ID ) OR c.CLIENT_ID = @Client_ID
	WHERE
		((CLIENT_NAME LIKE '%' + @Substring + '%'
		OR CLIENT_SHORTNAME LIKE '%' + @Substring + '%') AND (r.RelationshipType & 4 <> 0) AND r.RelationshipState = 1)
  END
END
GO