...
GO
CREATE PROCEDURE [dbo].[ClientsGetAutoComplete]
	@ID				int 
...
ELSE
  BEGIN
	...
	FROM 
		[dbo].[Clients] AS c
		INNER JOIN dbo.ClientRelationships cr ON cr.ParentClientId = @CLIENT_ID OR cr.PartnerClientId = @CLIENT_ID
			
	WHERE
      cr.RelationshipState = 1 AND (
		CLIENT_NAME LIKE '%' + @Substring + '%'
		OR CLIENT_SHORTNAME LIKE '%' + @Substring + '%')
  END
END
