...
GO
CREATE PROCEDURE [dbo].[InventoryReceiptsGetByClientIDByDateRange] 
	@ID			int
	, @From		datetime = NULL
	, @To		datetime = NULL
AS
BEGIN
	...
	WHERE 
		(i.[ClientID] = @ID)
		AND (@From IS NULL OR r.INVENTORYRECEIPT_RECEIVED >= @From)
		AND (@To IS NULL OR r.INVENTORYRECEIPT_RECEIVED <= @To)
