...
GO
CREATE PROCEDURE [dbo].[MailPieceGetSequenceRange2]
	@UserID			int
	, @SegmentID	int
	, @Range		int
AS
...
-- 2017-03-08 FJL - **** WE DON'T WANT TO DO THIS ****
--					We need the user's client ID
...
--WHERE
--	s.MAILSEGMENT_ID = @SegmentID;

-- 2017-03-08 FJL - **** DO THIS INSTEAD ****
SELECT
	@ClientID = u.USER_CLIENT_ID
FROM
	dbo.Users AS u
WHERE
...
FOR 
	SELECT CONVERT(int, [MailerId]), [IsDefaultTrackingId] FROM [usps].[Mailers] WHERE [ClientId] = @ClientID ORDER BY [MailerId];
...
BEGIN TRY
	BEGIN TRANSACTION
		...
		FETCH FIRST FROM mid_cursor INTO @MailerID, @IsDefault;
		...
		BEGIN  
			...
			FETCH NEXT FROM mid_cursor INTO @MailerID, @IsDefault;
		END   
		...
		FETCH ABSOLUTE @StartRow FROM mid_cursor INTO @MailerID, @IsDefault;
		WHILE @@FETCH_STATUS = 0  
		BEGIN  
		...
		-- 2017-03-08 FJL - **** WE DON'T WANT TO DO THIS.  ****
		--					They may be just getting additional sequences
		-- update segment est qty
		--UPDATE dbo.MailSegments
		--SET
		--	MAILSEGMENT_ESTQTY = @TotalSequences
		--WHERE
		--	MAILSEGMENT_ID = @SegmentID;

		-- insert our mailsegment sequences
