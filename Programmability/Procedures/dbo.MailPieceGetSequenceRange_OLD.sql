...
GO
CREATE PROCEDURE [dbo].[MailPieceGetSequenceRange_OLD]
	@MailerID	int -- = 106596
	-- @SegmentID	int
	, @Range	int
AS
...
--FOR 
--	SELECT MAILER_ID, MAILER_ISDEFAULTTRACKINGID FROM usps.Mailers WHERE MAILER_CLIENTID = @ClientID ORDER BY MAILER_ID;
...
--					UPDATE usps.Mailers SET MAILER_ISDEFAULTTRACKINGID = 0 WHERE MAILER_ID = @MailerID;
...
--							FETCH FIRST FROM mid_cursor INTO @MailerID, @IsDefault;
--						END
--					UPDATE usps.Mailers SET MAILER_ISDEFAULTTRACKINGID = 1 WHERE MAILER_ID = @MailerID;
--					BREAK;
--				END
