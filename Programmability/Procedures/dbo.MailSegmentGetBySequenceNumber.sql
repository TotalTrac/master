...
GO
CREATE PROCEDURE [dbo].[MailSegmentGetBySequenceNumber]
	@SequenceNumber	int
...
BEGIN
	...
	FROM
		...
		INNER JOIN [usps].[MailSegmentSequences] AS seq
			ON s.MAILSEGMENT_ID = seq.SEQUENCE_SEGMENTID
	WHERE
		@SequenceNumber BETWEEN seq.SEQUENCE_STARTVALUE AND seq.SEQUENCE_ENDVALUE
		AND
		(
