...
GO
CREATE PROCEDURE [dbo].[MailSegmentsGetUnbilledByClientId]
  @Id INT	
AS
BEGIN
	SET NOCOUNT ON;
...
UPDATE ms SET ms.MAILSEGMENT_BILLINGSTATUS = 1 
  ...
  WHERE ms.MAILSEGMENT_BILLINGSTATUS = 0
	...
	FROM 
      ...
      INNER JOIN dbo.Jobs j ON s.MAILSEGMENT_JOBID = j.JOB_ID
      INNER JOIN job.Jobs jj ON j.JOB_JOBID = jj.JOB_ID
      INNER JOIN dbo.Users u ON u.USER_USERID = jj.JOB_CREATEDBYID
	WHERE
		s.MAILSEGMENT_BILLINGSTATUS = 1 AND u.USER_CLIENT_ID = @Id
END
GO