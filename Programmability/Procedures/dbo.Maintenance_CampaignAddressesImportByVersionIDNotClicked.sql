...
GO
CREATE PROCEDURE [dbo].[Maintenance_CampaignAddressesImportByVersionIDNotClicked]	
	 @SourceVersionID	int
	, @TargetVersionID	int	
AS
BEGIN
	...
	WHERE 
		...
		AND (NOT EXISTS (		
			SELECT
				c.CAMPAIGNADDRESSCLICK_CAMPAIGNADDRESSID
			FROM	
				[dbo].[CampaignAddressClicks] AS c				
			WHERE
				c.CAMPAIGNADDRESSCLICK_CAMPAIGNADDRESSID = a.CAMPAIGNADDRESS_ID		
		))
		AND (NOT EXISTS (
			SELECT
