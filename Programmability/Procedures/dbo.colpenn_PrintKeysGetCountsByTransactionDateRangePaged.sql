...
GO
CREATE PROCEDURE [dbo].[colpenn_PrintKeysGetCountsByTransactionDateRangePaged] (  
	@Effort			char(1)		= NULL
...
BEGIN
	...
	(
		SELECT 
			ROW_NUMBER() OVER (ORDER BY t.PRINTKEY) AS RESULTNUM
			, t.PRINTKEY
			, t.EFFORT
			, CASE @GroupByDate WHEN 1 THEN BATCHDATE END AS BATCHDATE
			, CASE @GroupByDate WHEN 1 THEN MAILDATE END AS MAILDATE
		...
		(
			...
			WHERE 
				(@DateFrom IS NULL OR m.MAILRECORD_TRANSACTIONDATE >= @DateFrom) 
