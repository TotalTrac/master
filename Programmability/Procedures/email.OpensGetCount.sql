...
GO
CREATE PROCEDURE [email].[OpensGetCount] 
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	IF (@From IS NOT NULL OR @To IS NOT NULL) AND @GroupByDate = 1
		SELECT @sql = @sql + ', CONVERT(DATE, o.[DateTime]) AS [Date]';
	
	SELECT @sql = @sql + ', COUNT(*) AS [Count]
						 FROM [email].[Opens] AS o';
	...
	IF @From IS NOT NULL AND @To IS NOT NULL
		SELECT @sql = @sql + ' WHERE (CONVERT(DATE, o.[DateTime]) BETWEEN ''' + CONVERT(varchar, @From, 121) + ''' AND ''' + CONVERT(varchar, @To, 121) + ''')';
	ELSE IF @From IS NOT NULL
		SELECT @sql = @sql + ' WHERE (CONVERT(DATE, o.[DateTime]) >= ''' + CONVERT(varchar, @From, 121) + ''')';
	ELSE IF @To IS NOT NULL
		SELECT @sql = @sql + ' WHERE (CONVERT(DATE, o.[DateTime]) <= ''' + CONVERT(varchar, @To, 121) + ''')';
	...
	BEGIN
		...
		SELECT @sql = @sql + ' CONVERT(DATE, o.[DateTime]) ';
	END
