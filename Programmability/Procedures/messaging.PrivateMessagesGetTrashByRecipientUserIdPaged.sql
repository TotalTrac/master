...
CREATE PROCEDURE [messaging].[PrivateMessagesGetTrashByRecipientUserIdPaged]
	@Id int,
    @PageNumber INT = 1,
    @PageSize   INT = 10
AS
...
SET TRANSACTION ISOLATION LEVEL READ COMMITTED
  ...
  INNER JOIN messaging.RecipientLists rl ON rl.MessageId = sub.Id
      WHERE 	rl.RecipientUserId = @Id AND (sub.IsTrashRecipient = 1)
...
SELECT COUNT(pm.Id) AS TotalRows FROM messaging.PrivateMessages pm
  INNER JOIN messaging.RecipientLists rl2 ON rl2.MessageId = pm.Id
      WHERE 	rl2.RecipientUserId = @Id AND (pm.IsTrashRecipient = 1)
--endregion
