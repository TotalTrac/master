...
CREATE PROCEDURE [messaging].[PrivateMessagesGetTrashBySenderUserIdOrRecipientUserIdPaged]
	@Id int,
    @PageNumber INT = 1,
    @PageSize   INT = 10
AS
...
SET TRANSACTION ISOLATION LEVEL READ COMMITTED
  ...
  WHERE ((sub.IsTrashSender = 1  AND sub.SenderUserId = @Id)  OR (rl.RecipientUserId = @Id  AND sub.IsTrashRecipient = 1))
...
SELECT COUNT(pm.Id) AS TotalRows FROM messaging.PrivateMessages pm
  INNER JOIN messaging.RecipientLists rl2 ON rl2.MessageId = pm.Id
     WHERE ((pm.IsTrashSender = 1  AND pm.SenderUserId = @Id)  OR (rl2.RecipientUserId = @Id  AND pm.IsTrashRecipient = 1))
--endregion
GO