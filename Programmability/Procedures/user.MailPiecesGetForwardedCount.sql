...
GO
CREATE PROCEDURE [user].[MailPiecesGetForwardedCount]
	@UserID			int
	, @FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
	...
	SELECT ' +
		...
		+ ' AS [ID], COUNT(MAILPIECE_ID) AS [COUNT] 
			...
			INNER JOIN [dbo].[MailPieceScans] AS scan
				ON p.MAILPIECE_FORWARDSCANID = scan.MAILPIECESCAN_ID
		...
		SELECT @sql = @sql + 
			CASE @GroupBy
				WHEN 11 THEN
					' INNER JOIN [maponics].[ZipCodes] AS z ON x.MAILPIECE_ZIPCODE = z.ZIPCODE_ID '
				ELSE
					''
			END
