...
GO
CREATE PROCEDURE [usps].[MailPiecesGet] 
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	IF @From IS NOT NULL OR @To IS NOT NULL
		SELECT @sql = @sql + ' LEFT JOIN [dbo].[MailPieceScans] AS scan ON p.MAILPIECE_FINALIZESCANID = scan.MAILPIECESCAN_ID';
	...
	-- WHERE CLAUSE
	--SELECT @sql = @sql + ' WHERE p.MAILPIECE_FINALIZESCANID IS NOT NULL ';
