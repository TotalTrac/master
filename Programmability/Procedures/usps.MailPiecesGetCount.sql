...
GO
CREATE PROCEDURE [usps].[MailPiecesGetCount] 
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	SELECT @sql = 'SELECT ' +
		CASE @GroupBy
			...
			WHEN 10 THEN
				'LEFT(MAILPIECE_ZIPCODE, 3) + '' '' + MAILPIECE_STATE' -- 'LEFT(MAILPIECE_ROUTINGCODE, 3)'
			WHEN 11 THEN 
				'ZIPCODE_COUNTY'
			ELSE
				'NULL'
		...
		GROUP BY ' +
			CASE @GroupBy
				...
				WHEN 9 THEN
					'MAILPIECE_ZIPCODE' --'LEFT(MAILPIECE_ROUTINGCODE, 5)'
				WHEN 10 THEN
					'MAILPIECE_STATE, LEFT(MAILPIECE_ZIPCODE, 3)' -- 'LEFT(MAILPIECE_ROUTINGCODE, 3)'
				WHEN 11 THEN
					'ZIPCODE_COUNTY'
				ELSE
					'NULL'
