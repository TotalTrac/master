...
GO
CREATE PROCEDURE [usps].[MailPiecesGetFinalizedCalendar]
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	SELECT @sql = '	
		SELECT ' +
			CASE @GroupBy
				...
				ELSE
					'NULL'
			END + ' AS [ID]
			, MAILPIECE_DELIVERYDATE AS [DATETIME]
			, COUNT(*) AS [COUNT]			
		...
		SELECT @sql = @sql + ' WHERE MAILPIECE_DELIVERYDATE IS NOT NULL ';

		IF @From IS NOT NULL AND @To IS NOT NULL
			SELECT @sql = @sql + ' AND (MAILPIECE_DELIVERYDATE BETWEEN ''' + CONVERT(varchar, @From, 121) + ''' AND ''' + CONVERT(varchar, @To, 121) + ''')';
		ELSE IF @From IS NOT NULL
			SELECT @sql = @sql + ' AND (MAILPIECE_DELIVERYDATE >= ''' + CONVERT(varchar, @From, 121) + ''')';
		ELSE IF @To IS NOT NULL
			SELECT @sql = @sql + ' AND (MAILPIECE_DELIVERYDATE <= ''' + CONVERT(varchar, @To, 121) + ''')';
