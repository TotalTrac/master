...
GO
CREATE PROCEDURE [usps].[MailPiecesGetScannedCalendar]
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	SELECT @sql = '	
		SELECT ' +
			...
			FROM [dbo].[MailPieces] AS p
				INNER JOIN [dbo].[MailPieceScans] AS s
					ON p.MAILPIECE_FIRSTSCANID = s.MAILPIECESCAN_ID
		...
		--SELECT @sql = @sql + ' WHERE MAILPIECE_FIRSTSCANID IS NOT NULL ';

		IF @From IS NOT NULL AND @To IS NOT NULL
			SELECT @sql = @sql + ' WHERE (MAILPIECESCAN_DATETIME BETWEEN ''' + CONVERT(varchar, @From, 121) + ''' AND ''' + CONVERT(varchar, @To, 121) + ''')';
		ELSE IF @From IS NOT NULL
			SELECT @sql = @sql + ' WHERE (MAILPIECESCAN_DATETIME >= ''' + CONVERT(varchar, @From, 121) + ''')';
		ELSE IF @To IS NOT NULL
			SELECT @sql = @sql + ' WHERE (MAILPIECESCAN_DATETIME <= ''' + CONVERT(varchar, @To, 121) + ''')';
