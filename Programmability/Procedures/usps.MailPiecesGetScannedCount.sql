...
GO
CREATE PROCEDURE [usps].[MailPiecesGetScannedCount] 
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	IF @From IS NOT NULL OR @To IS NOT NULL
		SELECT @sql = @sql + ' INNER JOIN [dbo].[MailPieceScans] AS scan ON p.MAILPIECE_FIRSTSCANID = scan.MAILPIECESCAN_ID';
	...
	-- WHERE CLAUSE
	SELECT @sql = @sql + ' WHERE p.MAILPIECE_FIRSTSCANID IS NOT NULL ';
