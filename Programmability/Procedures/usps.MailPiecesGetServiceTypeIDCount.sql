...
GO
CREATE PROCEDURE [usps].[MailPiecesGetServiceTypeIDCount] 
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	SELECT @sql = 'SELECT ' +
		...
		END
		+ ' AS [ID], 
		SUBSTRING(MAILPIECE_TRACKINGCODE,3,3) AS [ID2],
		COUNT(*) AS [COUNT]
	...
	SELECT @sql = @sql + ' GROUP BY SUBSTRING(MAILPIECE_TRACKINGCODE,3,3)'
