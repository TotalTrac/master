...
GO
CREATE PROCEDURE [usps].[MailPiecesGetZipCodeTypeCount] 
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
	...
	, @From			datetime = NULL
	, @To			datetime = NULL
	-- still have to implement date range
AS
BEGIN
	-- @FilterBy
	-- 1 = Client
	...
	SELECT @sql = 'SELECT ' +
		CASE @GroupBy
			...
			WHEN 10 THEN
				'LEFT(MAILPIECE_ZIPCODE, 3)' -- 'LEFT(MAILPIECE_ZIPCODE, 3)' -- 'LEFT(MAILPIECE_ROUTINGCODE, 3)'
			WHEN 11 THEN 
				'ZIPCODE_COUNTY'
			ELSE
				'NULL'
		END
		+ ' AS [ID], 
		LEN(MAILPIECE_ROUTINGCODE) AS [ID2],
		COUNT(*) AS [COUNT]
	FROM [dbo].[MailPieces] AS p'
	
	IF ((@GroupBy IS NOT NULL AND @GroupBy <> 12) OR (@FilterBy IS NOT NULL AND @FilterBy <> 12) OR (@FilterBy2 IS NOT NULL AND @FilterBy2 = 11))
	...
	SELECT @sql = @sql + ' GROUP BY LEN(MAILPIECE_ROUTINGCODE)'
		...
		SELECT @sql = @sql + ', ' +
			CASE @GroupBy
				...
				WHEN 7 THEN
					'MAILPIECE_STATE'
				WHEN 8 THEN
					'MAILPIECE_ZIPCODE + MAILPIECE_ZIPCODE' --LEFT(MAILPIECE_ROUTINGCODE, 9)'
				WHEN 9 THEN
					'MAILPIECE_ZIPCODE' --'LEFT(MAILPIECE_ROUTINGCODE, 5)'
				WHEN 10 THEN
