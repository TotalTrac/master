...
GO
CREATE PROCEDURE [usps].[ReplyPiecesGetFinalizedAverage] 
	@FilterBy			int = NULL
	, @ID				int = NULL
	, @FilterBy2		int = NULL
...
BEGIN
	...
	SELECT @sql = '	
		SELECT ' +
			CASE @GroupBy
				...
				ELSE
					'NULL'
			END + ' AS [ID]
			, AVG(DATEDIFF(HOUR, MAILSEGMENT_DROPDATE, MAILPIECESCAN_DATETIME)) AS [COUNT]				
			FROM [dbo].[MailPieces] AS p
				...
				INNER JOIN dbo.Facilities AS f
					ON scan.MAILPIECESCAN_FACILITYID = f.FACILITY_ID
		...
		IF @PostalClassID IS NOT NULL
			SELECT @sql = @sql + ' AND MAILSEGMENT_POSTALCLASSID = ' + CONVERT(varchar, @PostalClassID);
