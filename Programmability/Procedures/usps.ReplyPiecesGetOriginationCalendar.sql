...
GO
CREATE PROCEDURE [usps].[ReplyPiecesGetOriginationCalendar]
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	SELECT @sql = '	
		SELECT ' +
			CASE @GroupBy
				...
				ELSE
					'NULL'
			END + ' AS [ID]
			, CONVERT (DATE,MAILPIECESCAN_DATETIME) AS [DATETIME]
			, COUNT(*) AS [COUNT]			
		...
		SELECT @sql = @sql + ' WHERE MAILPIECESCAN_DATETIME IS NOT NULL ';

			IF @From IS NOT NULL AND @To IS NOT NULL
				SELECT @sql = @sql + ' AND (CONVERT (DATE,MAILPIECESCAN_DATETIME) BETWEEN ''' + CONVERT(varchar, @From, 121) + ''' AND ''' + CONVERT(varchar, @To, 121) + ''')';
			ELSE IF @From IS NOT NULL
				SELECT @sql = @sql + ' AND (CONVERT (DATE,MAILPIECESCAN_DATETIME) >= ''' + CONVERT(varchar, @From, 121) + ''')';
			ELSE IF @To IS NOT NULL
				SELECT @sql = @sql + ' AND (CONVERT (DATE,MAILPIECESCAN_DATETIME) <= ''' + CONVERT(varchar, @To, 121) + ''')';
		...
		GROUP BY ' +
			CASE @GroupBy
				...
				ELSE
					'NULL'
			END
		+ ', CONVERT (DATE,MAILPIECESCAN_DATETIME)';
