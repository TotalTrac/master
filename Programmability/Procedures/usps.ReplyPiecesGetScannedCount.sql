...
GO
CREATE PROCEDURE [usps].[ReplyPiecesGetScannedCount] 
	@FilterBy		int = NULL
	, @ID			int = NULL
	, @FilterBy2	int = NULL
...
BEGIN
	...
	--	SELECT @sql = @sql + ' INNER JOIN [dbo].[MailPieceScans] AS scan ON p.MAILPIECE_FINALIZESCANID = scan.MAILPIECESCAN_ID';

	--IF @GroupBy = 11 OR @FilterBy = 11 OR @FilterBy2 = 11 OR
	--	@GroupBy = 5 OR @GroupBy = 6 OR @GroupBy = 7 OR
	IF @GroupBy > 4 OR
